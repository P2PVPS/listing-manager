...
async function addPaymentObject(config) {
  ...
  console.log(`inspecting config object: ${util.inspect(config)}`);

  const orderId = config.obNotice.notification.orderId;

  const orderInfo = await openbazaar.getOrder(config, orderId);

  console.log(`inspecting orderInfo: ${util.inspect(orderInfo)}`);

  // Payment in Satoshis.
  const contractPrice = orderInfo.paymentAddressTransactions[0].value;

  // Subtract fee
  const fee = Math.floor(((contractPrice * FEE_PERCENTAGE) / 100) * 1.0);
  const net = contractPrice - fee;

  console.log(`contractPrice: ${contractPrice}, fee: ${contractPrice}, net: ${net}`);
